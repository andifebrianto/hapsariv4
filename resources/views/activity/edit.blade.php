...
@section('container')
    ...
    <!-- Activity Section -->
    <div class="container col-xxl-8 py-5" style="margin-left:15%; margin-right:15%">
        <div class="card bg-white p-4 shadow rounded-4 border-0">
            {{-- <h1>Edit {{ $kategori }}</h1>
            <p class="mb-4">
                <a href="/activity?kategori={{ $kategori }}#activity" class="text-decoration-none text-dark">Activity</a> / Create {{ $kategori }}
            </p> --}}
            <div class="section-header mb-6">
                <h2>Change {{ $kategori }}</h2>
            ...
            <form action="/activity/{{ $activity->id }}" method="POST" enctype="multipart/form-data">
                ...
                <input type="hidden" name="kategori" value="{{ $activity->kategori }}">
                <input type="hidden" name="thumbnail_id" value="{{ $activity->thumbnail_id }}">

                <div class="form-group mb-3">
                    <label><strong>{{ $kategori }} Title</strong></label>
                    <input type="text" class="form-control @error('judul') is-invalid @enderror" name="judul" required autofocus value="{{ $activity->judul }}" placeholder="Input title">
                    @error('judul')
                ...
                <div class="form-group mb-3">
                    ...
                    <textarea name="info" class="form-control @error('info') is-invalid @enderror" style="height: 100px" required>{{ $activity->info }}</textarea>
                    @error('info')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <div class="py-2">
                        <img class="img-preview" id="frame" width="300" src="{{ asset('storage/activities/' . $activity->image) }}">
                    </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <textarea name="desc" id="summernote" class="form-control  @error('desc') is-invalid @enderror" required>
                        {{ $activity->desc }}
                    </textarea>
                    @error('desc')
                        <div class="invalid-feedback">
